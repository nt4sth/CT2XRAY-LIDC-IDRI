...
if __name__ == '__main__':
    root_path = './data/Raw-LIDC-IDRI/'
    mask_root_path = './data/Mask-LIDC-IDRI/'
    ...
    for fileIndex, filePath in enumerate(files_list):
        try:
            ...
            std_scan = crop_to_standard(new_scan, scale=320) # 3维CT图像的分辨率是320x320x320
            save_scan_mhda(std_scan, (0, 0, 0), new_spacing, savePath)
            _, _, _, new_size, _ = load_scan_mhda(savePath)
            print("New : ", new_size)
            t1 = time.time()
